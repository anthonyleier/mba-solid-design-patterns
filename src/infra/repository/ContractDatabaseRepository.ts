...
export default class ContractDatabaseRepository implements ContractRepository {
    ...
    async list(): Promise<Contract[]> {
        ...
        const contractsData = await this.connection.query("SELECT * FROM cruz.contract", []);
        ...
        for (const contractData of contractsData) {
            ...
            const paymentsData = await this.connection.query("SELECT * FROM cruz.payment WHERE id_contract = $1", [contract.idContract]);
